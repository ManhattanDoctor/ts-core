...
export class TransportAmqp extends Transport {
  ...
  public send<U>(command: ITransportCommand<U>): void {
    if (_.isNil(command.request)) {
      throw new ExtendedError(
        `${command.name} command request is null or undefined`
      );
    }
  ...
  private createCommandOptions<U>(
    ...
    let expiration = _.isNumber(options.waitTimeout)
      ? options.waitTimeout
      : Transport.WAIT_TIMEOUT;
    ...
    return {
      ...
      headers: {
        GATEWAY_TRANSPORT_TIMEOUT: expiration,
  ...
  public listen<U>(commandName: string): Observable<U> {
    ...
    this.listenQueue(
      ...
      (msg: Message) => {
        ...
        if (
          _.isNil(headers) ||
          _.isNil(headers.IS_NEED_REPLY) ||
          _.isNil(headers.IS_ASYNC_COMMAND)
        ) {
  ...
  public wait<U>(command: ITransportCommand<U>): void {
    ...
    let headers = msg.properties.headers as ICommandHeaders;
    let timeout = _.isNil(headers.GATEWAY_TRANSPORT_TIMEOUT)
      ? TransportAmqp.WAIT_TIMEOUT
      : headers.GATEWAY_TRANSPORT_TIMEOUT;

    this.reject(msg);
    if (
      this.isCommandAsync(command) &&
      waitCount * TransportAmqp.DELAY_TTL > timeout
    ) {
      throw new TransportWaitError(
        `Wait timeout exceed ${command.name} (${command.id})`
      );
    }
  ...
  private async reconnect(): Promise<void> {
    ...
    try {
      ...
      if (
        this.connectionAttempts * TransportAmqp.RECONNECT_DELAY >=
        TransportAmqp.CONNECTION_TIMEOUT
      ) {
  ...
  private async startListenReply<U, V>(
    queueName: string,
    command: ITransportCommandAsync<U, V>
  ) {
  ...
  private listenReply<U, V>(
    msg: Message,
    command: ITransportCommandAsync<U, V>
  ) {
    if (_.isNil(msg.properties.correlationId)) {
      this.error(
        `CorrelationId not found, messageId=${msg.properties.messageId}`
      );
    ...
    let newCommand = new TransportCommandAsync<any, any>(
      command.name,
      command.request,
      command.id
    );
  ...
  private transformNullOrUndefined(msg: Message, data): any {
    if (
      ...
      msg.properties.headers[RMQ_HEADER.GATEWAY_NULL_RESPONSE]
    ) {
      return null;
    }
    if (
      msg.properties.headers &&
      msg.properties.headers[RMQ_HEADER.GATEWAY_UNDERFINED_RESPONSE]
    ) {
      return undefined;
    }
    return data;
  ...
  private getMessage<U, V>(command: ITransportCommand<U>): Message {
    ...
    if (!msg) {
      throw new ExtendedError(
        `AMQP Message for command ${command.id} not found`
      );
    }
    return msg;
  }
  ...
  private rejectError(
    msg: Message,
    data: any,
    promise: PromiseHandler<any, ExtendedError>
  ) {
    if (
      msg.properties.headers &&
      msg.properties.headers[RMQ_HEADER.GATEWAY_TRANSPORT_ERROR]
    ) {
      promise.reject(this.transformError(data));
    }
  }

  private async sendToQueue<U>(
    command: ITransportCommand<U>,
  ...
  private async sendReplyToQueue<U, V>(
    command: ITransportCommandAsync<U, V>,
  ...
  private async consumeEvent(): Promise<void> {
    ...
    this.channel
      .consume(
        this.eventQueueName,
        (msg: Message) => {
          const event = JSON.parse(msg.content.toString()) as ITransportEvent<
            any
          >;
  ...
  private getRetry(msg: Message): number {
    return !msg.properties.headers || !msg.properties.headers["x-death"]
      ? 0
      : parseInt(msg.properties.headers["x-death"][0].count.toString(), 10);
  }
